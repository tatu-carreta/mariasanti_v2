        ...
        @if(Auth::check())
            @if(Auth::user()->can("ordenar_seccion_estatica"))
                @if(count($menu_basic->seccionesConItems()) >= 2)
                    <div class="row">
                        <div class="col-md-12">
                            <a href="{{URL::to('admin/seccion/ordenar-por-menu/'.$menu_basic->id)}}" class="btn pull-right popup-nueva-seccion"><i class="fa fa-exchange fa-lg"></i>Ordenar secciones</a>
                        </div>
                    </div>
            ...
            <div class="row">
                <div class="col-md-12 fondoDestacado text-center menuInsertContenido">       
                    ...
                    @if(Auth::user()->can("agregar_texto"))
                        <a href="{{URL::to('admin/texto/agregar/'.$menu_basic->id)}}" class="btn btn-primary popup-nueva-seccion">Agregar texto</a> 
                    @endif
                    @if(Auth::user()->can("agregar_html"))
                        <a href="{{URL::to('admin/html/agregar/'.$menu_basic->id)}}" class="btn btn-primary popup-nueva-seccion">Agregar HTML</a>
                    @endif
                </div>
            </div>
            <div class="clearfix"></div>
